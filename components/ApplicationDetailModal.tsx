...
const DetailSection: React.FC<{ title: string; children: React.ReactNode; icon?: React.ReactNode }> = ({ title, children, icon }) => (
    <div className="mb-6">
        <h3 className="text-base font-semibold text-white mb-3 pb-2 border-b border-white/10 flex items-center gap-2">
            {icon}
            {title}
        </h3>
        {children}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-4xl max-h-[90vh] overflow-hidden">
                <GlassCard className="p-0" glow="blue">
                    ...
                    {/* Header */}
                    <div className="sticky top-0 z-10 flex items-center justify-between p-5 border-b border-white/10 bg-white/[0.02] backdrop-blur-xl">
                    ...
                    <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
                        ...
                        >
                            <GlassCard className="p-4 space-y-3">
                                <div>
                                    <span className="text-sm text-white/50">計畫名稱</span>
                                    <p className="text-white font-medium">{application.title}</p>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <span className="text-sm text-white/50">申請日期</span>
                                        <p className="text-white/80">{application.applyDateStart}</p>
                                    </div>
                                    <div>
                                        <span className="text-sm text-white/50">審核狀態</span>
                                        <div className="mt-1">
                                    ...
                                    {application.members.map((member, index) => (
                                        <GlassCard key={index} className="p-4">
                                            <div className="flex items-center gap-2 mb-3">
                                                ...
                                                <span className="font-medium text-white">{index === 0 ? '組長' : `組員 ${index}`}</span>
                                            </div>
                                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                                                {member.student_name && (
                                                    <div>
                                                        <span className="text-white/50">姓名</span>
                                                        <p className="text-white/80">{member.student_name}</p>
                                                    </div>
                                                )}
                                                <div>
                                                    <span className="text-white/50">學號</span>
                                                    <p className="text-white/80">{member.student_id}</p>
                                                </div>
                                                <div>
                                                    <span className="text-white/50">班級</span>
                                                    <p className="text-white/80">{member.student_class}</p>
                                                </div>
                                                <div>
                                                    <span className="text-white/50">座號</span>
                                                    <p className="text-white/80">{member.student_seat}</p>
                                                </div>
                                            </div>
                                        </GlassCard>
                                    ))}
                                    ...
                                    {application.references.map((ref, index) => (
                                        <GlassCard key={index} className="p-4">
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                                                <div>
                                                    <span className="text-white/50">書名</span>
                                                    <p className="text-white/80">{ref.book_title}</p>
                                                </div>
                                                <div>
                                                    <span className="text-white/50">作者</span>
                                                    <p className="text-white/80">{ref.author}</p>
                                                </div>
                                                <div>
                                                    <span className="text-white/50">出版社</span>
                                                    <p className="text-white/80">{ref.publisher}</p>
                                                </div>
                                                {ref.link && (
                                                    <div>
                                                        <span className="text-white/50">連結</span>
                        ...
                        {application.expected_outcome && (
                            ...
                            >
                                <GlassCard className="p-4">
                                    <p className="text-white/80 whitespace-pre-wrap">{application.expected_outcome}</p>
                                </GlassCard>
                            </DetailSection>
                        )}
                        ...
                        {application.equipment_needs && (
                            ...
                            >
                                <GlassCard className="p-4">
                                    <p className="text-white/80 whitespace-pre-wrap">{application.equipment_needs}</p>
                                </GlassCard>
                            </DetailSection>
                        )}
                                    ...
                                    {application.plan_items.map((item, index) => (
                                        <GlassCard key={index} className="p-4">
                                            ...
                                            <div className="grid grid-cols-2 gap-3 text-sm mb-3">
                                                ...
                                                <div>
                                                    <span className="text-white/50">時數</span>
                                                    <p className="text-white/80">{item.hours} 小時</p>
                                                </div>
                                            </div>
                                            <div className="space-y-2 text-sm">
                                                <div>
                                                    <span className="text-white/50">學習內容</span>
                                                    <p className="text-white/80">{item.content}</p>
                                                </div>
                                                <div>
                                                    <span className="text-white/50">檢核指標</span>
                        ...
                        {application.midterm_goal && (
                            ...
                            >
                                <GlassCard className="p-4">
                                    <p className="text-white/80 whitespace-pre-wrap">{application.midterm_goal}</p>
                                </GlassCard>
                            </DetailSection>
                        )}
                        ...
                        {application.final_goal && (
                            ...
                            >
                                <GlassCard className="p-4">
                                    <p className="text-white/80 whitespace-pre-wrap">{application.final_goal}</p>
                                </GlassCard>
                            </DetailSection>
                        )}
                    ...
                    {/* Footer */}
                    <div className="sticky bottom-0 flex justify-end p-5 border-t border-white/10 bg-white/[0.02] backdrop-blur-xl">
