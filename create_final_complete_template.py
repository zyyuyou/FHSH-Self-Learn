...
try:
    ...
    # 是否繳交過 - 加入打勾邏輯
    if len(member1_row.cells) > 9:
        cell = member1_row.cells[9]
        for para in cell.paragraphs:
            para.clear()
        if len(cell.paragraphs) == 0:
            cell.add_paragraph()
    ...
    # 是否繳交過
    if len(member2_row.cells) > 9:
        cell = member2_row.cells[9]
        for para in cell.paragraphs:
            para.clear()
        if len(cell.paragraphs) == 0:
            cell.add_paragraph()
        text = (
            "{% if member2.has_submitted == '是' %}☑{% else %}□{% endif %}是  "
            "{% if member2.has_submitted == '第一次申請' %}☑{% else %}□{% endif %}第一次申請\n"
            "{% if member2.has_submitted == '否' %}☑{% else %}□{% endif %}否"
        )
        cell.paragraphs[0].add_run(text)
    print("✓ 第4行: 組員2資訊（含是否繳交過打勾）")
    ...
    # 是否繳交過
    if len(member3_row.cells) > 9:
        cell = member3_row.cells[9]
        for para in cell.paragraphs:
            para.clear()
        if len(cell.paragraphs) == 0:
            cell.add_paragraph()
        text = (
            "{% if member3.has_submitted == '是' %}☑{% else %}□{% endif %}是  "
            "{% if member3.has_submitted == '第一次申請' %}☑{% else %}□{% endif %}第一次申請\n"
            "{% if member3.has_submitted == '否' %}☑{% else %}□{% endif %}否"
        )
        cell.paragraphs[0].add_run(text)
    print("✓ 第5行: 組員3資訊（含是否繳交過打勾）")
    ...
    # ===== 第13-21行: 學習內容規劃 (9個項次) =====
    # 注意：日期格式轉換已在後端PDF服務中處理（YYYY-MM-DD → MM/DD）
    print("\n新增學習內容規劃...")
    for idx in range(9):
        row_num = 13 + idx
    ...
    print(f"  4. ✅ 「是否繳交過」增加打勾邏輯（是/第一次申請/否）")
    print(f"  5. ✅ 日期格式轉換為 MM/DD（已在後端處理）")
    print(f"  6. ✅ 手機使用規範增加打勾邏輯（同意/不同意）")
    print(f"\n下一步: 複製到Docker容器")
